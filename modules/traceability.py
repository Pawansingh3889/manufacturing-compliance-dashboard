...
def trace_batch(batch_code):
    ...
    # Check if it's a raw material batch (RM-YYMMDD-NNNN)
    if batch_code.startswith("RM"):
        rm = query("SELECT * FROM raw_materials WHERE batch_code = :b", {"b": batch_code})
        if not rm.empty:
            result["found"] = True
            result["raw_materials"] = rm
            # Find production runs using this raw material
            prod = query("SELECT p.*, pr.name as product_name FROM production p JOIN products pr ON p.product_id = pr.id WHERE p.raw_material_batch = :b", {"b": batch_code})
            result["production"] = prod
            # Find orders linked to those production batches
            if not prod.empty:
                prod_batches = prod["batch_code"].tolist()
                placeholders = ",".join([f"'{b}'" for b in prod_batches])
                orders = query(f"SELECT o.*, pr.name as product_name FROM orders o JOIN products pr ON o.product_id = pr.id WHERE o.production_batch IN ({placeholders})")
                result["orders"] = orders
            else:
                result["orders"] = pd.DataFrame()
            return result

    # Check if it's a production batch (PR-YYMMDD-NNNN)
    if batch_code.startswith("PR"):
        prod = query("SELECT p.*, pr.name as product_name FROM production p JOIN products pr ON p.product_id = pr.id WHERE p.batch_code = :b", {"b": batch_code})
        if not prod.empty:
            result["found"] = True
            result["production"] = prod
            ...
            orders = query("SELECT o.*, pr.name as product_name FROM orders o JOIN products pr ON o.product_id = pr.id WHERE o.production_batch = :b", {"b": batch_code})
            result["orders"] = orders
            return result
